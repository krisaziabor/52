...
export default function ArtistStatements({ artistNames, artistsData }: ArtistStatementsProps) {
  ...
  const activeArtistStatements = useMemo(() => artistsData[activeArtist] || [], [artistsData, activeArtist]);
  
  // Find the active statement content
  const activeStatementContent = activeArtistStatements.find(
  ...
  const advanceAutoplay = useCallback(() => {
    ...
    if (activeArtist) {
      ...
      const availableStatements = artistsData[activeArtist]?.filter(s => s.isComplete) || [];
      ...
      const currentStatementIndex = availableStatements.findIndex(s => s.number === activeStatement);
      
      // Move to the next statement if available
      if (currentStatementIndex >= 0 && currentStatementIndex < availableStatements.length - 1) {
        const nextStatement = availableStatements[currentStatementIndex + 1].number;
  ...
  }, [activeArtist, activeStatement, autoplayActive, artistNames, artistsData, setHasStartedAutoplay]);
  ...
  useEffect(() => {
    ...
    document.addEventListener('selectArtist', handleSidebarSelection as EventListener);
    ...
    return () => {
      document.removeEventListener('selectArtist', handleSidebarSelection as EventListener);
  ...
  useEffect(() => {
    ...
    const startAutoplayForCurrent = () => {
      ...
      if (activeStatement) {
        const statement = activeArtistStatements.find(s => s.number === activeStatement);
  ...
  }, [activeArtist, activeStatement, autoplayActive, hasStartedAutoplay, activeArtistStatements, calculateReadingTime, advanceAutoplay]);
  ...
  const handleSVGStatementSelect = (statementNumber: string) => {
    const statement = activeArtistStatements.find(s => s.number === statementNumber);
    if (statement && statement.isComplete) {
          ...
          {artistNames.map((name, index) => (
            ...
            >
              <span className="block lowercase group">
                <span className={`${activeArtist === name ? 'hidden' : 'block'}`}>
                  {name}
                </span>
                <span className={`${activeArtist === name ? 'block' : 'hidden'} text-5xl`}>
                  {name}
                </span>
              </span>
            </li>
